...
#   nmr_soil.csv      — 19-node soil hydraulic properties (Julia's own soil
#                        profile, built directly at NMR_DEP19_CM -- the 10
#                        real nodes + 9 midpoints; odd 1-indexed positions
#                        are the real nodes, matching NicheMapR's own
#                        pedotransfer() convention). PE/KS/BB/BD/DD/L use all
#                        19; DEP/soilprops (the 10-node temperature grid)
#                        subset to the odd positions -- see below. Already
#                        includes the organic-cap override on Thcond/SpecHeat
#                        for the top nodes (see pipeline.jl's prepare_site) --
#                        no separate override here, single source of truth.
#   nmr_initial.csv   — initial soil T and SM at the 10 real nodes
#
# Calls NicheMapR::microclimate() directly (the Fortran solver) with Julia's
# identical forcing, bypassing micro_aust/micro_ncep's own internal
# AWAP/NCEP OPeNDAP fetchers -- same approach as scan_snotel/run_nmr.R, so
# both models see exactly the same weather.
#
# No snow (OzNet's Murrumbidgee catchment sites don't see persistent snow):
# snowmodel = 0, which uses microrun.R's simpler direct-fill soilinit layout
# (`soilinit <- c(Soil_Init, rep(avetemp, 10))`, positions 1:10 = actual
# initial soil temps) rather than scan_snotel's snowmodel=1 8-padding layout.
#
# Writes to outdir:
#   metout.csv, soil.csv, soilmoist.csv
...
RHMAXX   <- forcing$RHmax_pct
RHMINN   <- forcing$RHmin_pct
CCMAXX   <- forcing$CCmax_pct
CCMINN   <- forcing$CCmin_pct
WNMAXX   <- forcing$Wind_ms       # already at 2 m -- no height correction
WNMINN   <- WNMAXX * 0.1          # impose diurnal cycle, matching micro_global.R
...
maxpool  <- 10000
evenrain <- 0
snowmodel <- 0   # no snow -- see header comment
writecsv <- 0
hourly   <- 0
rainhourly <- 0
lamb     <- 0
...
# ── 5. Snow parameters (inert -- snowmodel = 0) ──────────────────────────────
snowtemp   <- 1.5
snowdens   <- 0.375
snowmelt   <- 1
undercatch <- 1
...
# ── 6. Soil properties ────────────────────────────────────────────────────────
# soil10 is 19-node (NMR_DEP19_CM); the 10-node temperature grid (DEP,
# soilprops) uses just the odd (real-node) positions, matching NicheMapR's
# own pedotransfer() convention (e.g. micro_silo.R's `BD[seq(1,19,2)]`).
real_node <- seq(1, 19, 2)
DEP <- soil10$DEP[real_node]

soilprops <- matrix(data = 0, nrow = 10, ncol = 5)
soilprops[, 1] <- soil10$BulkDensity[real_node]
soilprops[, 2] <- 1 - soilprops[, 1] / soil10$MineralDensity[real_node]
soilprops[soilprops[, 2] < 0.26, 2] <- 0.26
soilprops[, 3] <- soil10$Thcond[real_node]
soilprops[, 4] <- soil10$SpecHeat[real_node]
soilprops[, 5] <- soil10$MineralDensity[real_node]

PE <- soil10$PE
KS <- soil10$KS
BB <- soil10$BB
BD <- soil10$BulkDensity
DD <- soil10$MineralDensity
L  <- soil10$L
...
# snowmodel=0 layout: Soil_Init occupies positions 1:10 directly, padded to
# 20 total (matches micro_global.R's `soilinit <- c(Soil_Init, rep(avetemp,
# 10))` for snowmodel == 0 -- see header comment).
...
# NicheMapR::microclimate() hard-checks length(microinput) == 75.
microinput <- c(
